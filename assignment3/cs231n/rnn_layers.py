...
def rnn_step_backward(dnext_h, cache):
  ...
  dx = (dnext_h * dv).dot(Wx.T)
  dprev_h = (dnext_h * dv).dot(Wh.T)
  dWx = x.T.dot(dnext_h * dv)
  dWh = prev_h.T.dot(dnext_h * dv)
  db = np.sum(dnext_h * dv,axis = 0)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def word_embedding_backward(dout, cache):
  ...
  for n in range(N):
    for t in range(T):
      # for each dout[:,:,D], dW[:.D] correspondingly ++
      # W[x[n,t]] is the index of V to D,each (D,) size
      dW[x[n,t]] += dout[n,t]
